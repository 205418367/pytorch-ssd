...
parser.add_argument('--threshold', default=0.11, type=float, help='score threshold')
parser.add_argument('--imgs_path', default="/home/lichen/datasets/WIDER_val/images", type=str, help='imgs dir')
parser.add_argument('--result_path', default="widerface_evaluation/", type=str, help='imgs dir')
...
def inference():
    ...
    counter = 0
    for parent, dir_names, file_names in os.walk(args.imgs_path):
        for file_name in file_names:
            if not file_name.lower().endswith('jpg'):
               continue
    
            img_path = os.path.join(parent, file_name)
            ...
            if not os.path.exists(os.path.join(args.result_path, event_name)):
               os.makedirs(os.path.join(args.result_path, event_name))
            fout = open(os.path.join(args.result_path, event_name, file_name.split('.')[0] + '.txt'), 'w')
            fout.write(file_name.split('.')[0] + '\n')
            fout.write(str(boxes.shape[0]) + '\n')
